create database practica_bd

ON
primary
	(name = practicasprimary,
	filename = 'F:\Users\Marco\Documents\SQL server\MSSQL14.MSSQLSERVER\MSSQL\DATA\Practicas_db\Practica_1\practica1_primary.mdf',
	size = 50MB,
	Maxsize = 200MB,
	filegrowth = 20),
filegroup practicasFG
	(name = practicasData,
	filename = 'F:\Users\Marco\Documents\SQL server\MSSQL14.MSSQLSERVER\MSSQL\DATA\Practicas_db\Practica_1\practica1_Data.ndf',
	size = 200MB,
	Maxsize = 800MB,
	filegrowth = 100) 
...
Log on
	(name = practicaslog,
	filename = 'F:\Users\Marco\Documents\SQL server\MSSQL14.MSSQLSERVER\MSSQL\DATA\Practicas_db\Practica_1\practica1_log.mdf',
	size = 50MB,
	Maxsize = 200MB,
	filegrowth = 20)
...
--Products Table
-- Start with Tables that don't have foreign key
...
-- Offices table
-- Foreign key with MGR EMPL_NUM.salesreps remains to be declared
...
CREDIT_LIMIT MONEY,
 ...
 CONSTRAINT ISFOR FOREIGN KEY (MFR, PRODUCT)
  ...
  -- ALTER TABLE to add pending relations
  ALTER TABLE OFFICES
...
--Delete previously loaded rows in the Tables
...
-- OP 0
SELECT EM.NOMBRE
FROM EMPRESA EM, TRABAJA T
WHERE T.IDEMPRESA = EM.IDEMPRESA
GROUP BY EM.IDEMPRESA
HAVING COUNT(T.IDMEPLEADO) >= ALL	(	SELECT	COUNT(T2.IDEMPLEADO)
										FROM	TRABAJA T2
										GROUP BY	T2.IDEMPRESA)
--OP 1
SELECT P.MFR_ID, COUNT(*)
FROM PRODUCTS P
GROUP BY P.MFR_ID
...
HAVING COUNT(*) =  ( SELECT MAX(CANTIDAD)
						...
						FROM PRODUCTS P2
						GROUP BY P2.MFR_ID)AUX)




-- a) Show the sales (TARGET) and sales (SALES) objectives for each city.

SELECT CITY, OFFICE, TARGET, SALES
FROM OFFICES
ORDER BY CITY

-- b) 

SELECT OFFICE,CITY,SALES, TARGET
FROM OFFICES
WHERE REGION = 'EASTERN'
	AND
	SALES > TARGET

-- c) Show the sales representatives whose sales quota (QUOTA) is
-- greater than that of their manager (MANAGER).

SELECT NAME, EMPL_NUM
FROM SALESREPS AS S
WHERE S.QUOTA > (SELECT QUOTA
				FROM SALESREPS AS S2
				WHERE S2.EMPL_NUM = S.MANAGER)

SELECT S.NAME, S.EMPL_NUM
FROM SALESREPS AS S, SALESREPS AS S2
WHERE S.QUOTA > S2.QUOTA
		AND	S.MANAGER = S2.EMPL_NUM

-- F) * Show all the information from the office table.
...
--Cross product between SALESREPS and OFFICES
...
SELECT DISTINCT MGR --ELIMINAMOS EL 108 REPERIDO
FROM OFFICES
...
WHERE P.PRICE > 2000
UNION
SELECT DISTINCT MFR, PRODUCT --Avoid repetition
FROM ORDERS O
...
(SELECT MFR_ID, PRODUCT_ID
FROM PRODUCTS P
WHERE (P.QTY_ON_HAND * P.PRICE) > 30000)
...
SELECT DESCRIPTION, MFR_ID,QTY_ON_HAND
FROM PRODUCTS
WHERE DESCRIPTION LIKE '%lb%'
...
SELECT * 
FROM SALESREPS S
WHERE S.EMPL_NUM IN (101,108,115)
ORDER BY SALES
...
SELECT * 
FROM SALESREPS S
WHERE S.REP_OFFICE IS NULL --> WHERE S.REP_OFFICE = NULL
...
-- Q)
SELECT COUNT (  S.REP_OFFICE)
FROM SALESREPS S
WHERE S.SALES > S.QUOTA

SELECT   S.REP_OFFICE
FROM SALESREPS S
WHERE S.SALES > S.QUOTA

SELECT COUNT ( DISTINCT S.REP_OFFICE)
FROM SALESREPS S
WHERE S.SALES > S.QUOTA

-- R) GROUP BY
-- Group by Office and Count 
SELECT S.REP_OFFICE, COUNT(*)
FROM SALESREPS S
GROUP BY S.REP_OFFICE

-- S) GROUP BY

SELECT C.CUST_REP, COUNT(*)
FROM CUSTOMERS C
GROUP BY C.CUST_REP

-- T) Compare the result
--OP1
SELECT O.REP, SUM(O.AMOUNT)
FROM ORDERS O
GROUP BY O.REP
--OP2
...
SELECT S.REP_OFFICE, SUM(S.SALES)
FROM SALESREPS S
GROUP BY S.REP_OFFICE
HAVING COUNT(S.REP_OFFICE) > 1
...
FROM SALESREPS S INNER JOIN OFFICES O
ON
REP_OFFICE = OFFICE

-- W)

SELECT O.ORDER_NUM, P.PRODUCT_ID, O.QTY, P.DESCRIPTION
FROM ORDERS O, PRODUCTS P 
WHERE O.MFR =P.MFR_ID
		AND O.PRODUCT = P.PRODUCT_ID

--NATURAL JOIN needs the features to have the same name 

--X
-- Worker and their city
SELECT NAME, CITY
FROM SALESREPS JOIN OFFICES
ON REP_OFFICE = OFFICE

-- With left join the rows of the left table are part if the result
-- even when they cannot be matched with any row of the right table
...
SELECT OFFICE, SALES, TARGET, (SALES- TARGET)
FROM OFFICES
ORDER BY (SALES - TARGET) DESC

-- 2 

SELECT OFFICE,REGION,(SALES-TARGET)
FROM OFFICES
...
SELECT O.ORDER_NUM, S.EMPL_NUM, C.CUST_NUM
FROM SALESREPS S, CUSTOMERS C, ORDERS O
WHERE O.CUST = C.CUST_NUM
	AND O.REP = S.EMPL_NUM
...
SELECT ORDER_NUM, MFR, DESCRIPTION, QTY, AMOUNT
...
FROM ORDERS 
WHERE ORDER_DATE > '2008-01-01 00:00:00'
	AND AMOUNT > 10000
ORDER BY ORDER_DATE


--
--					Not so basic Queries
--
...
FROM OFFICES O
WHERE O.TARGET > (SELECT SUM(QUOTA)
					FROM SALESREPS
					WHERE REP_OFFICE = O.OFFICE)

-- 2 

SELECT C.CUST_NUM
FROM CUSTOMERS C
WHERE C.CUST_REP = (SELECT S.EMPL_NUM
					FROM SALESREPS S
					WHERE S.NAME = 'Bill Adams')

-- 3

SELECT S.NAME
FROM SALESREPS S
WHERE S.REP_OFFICE IN (SELECT O.OFFICE
						FROM OFFICES O
						WHERE O.SALES > O.TARGET)
